...
type (
	...
	Handler func(*Context) common.Error
...
func (r *Router) PreHandler(handlers ...Handler) {
...
func (r *Router) HandleRequest(ctx *fasthttp.RequestCtx) {
	...
	c.init(ctx)
	if len(r.preHandlers) > 0 {
		for _, handler := range r.preHandlers {
...
func (r *Router) handleError(c *Context, err common.Error) {
	marshal, _ := json.Marshal(err)
	c.Success("application/json", marshal)
...
func NotFoundHandler(*Context) common.Error {
	return common.NewError(http.StatusNotFound)
}
...
func MethodNotAllowedHandler(c *Context) common.Error {
